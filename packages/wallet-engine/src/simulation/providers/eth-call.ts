 ...
 *
 * Layer 1 (P0) simulation: checks for revert, returns revert reason
 * if the transaction would fail. Cannot resolve balance changes,
 ...
 *
 * Solidity errors: Error(string) → 4 byte selector (08c379a0) + offset(32B) + length(32B) + data
 * Panic: Panic(uint256) → 4 byte selector (4e487b71) + code(32B)
 */
...
/**
 * Build a safe SimulationResult when eth_call does not revert.
 */
function buildSuccessResult(
  ...
  return {
    ...
    summary: `Transaction simulation succeeded (basic revert check only, native: ${nativeSymbol})`,
...
export class EthCallProvider implements SimulationProvider {
  ...
  readonly supportedChains: number[] = []; // empty = all EVM
   ...
   *
   * CANNOT resolve balance changes or approval changes.
   */
  async simulate(
    ...
    try {
      ...
      return buildSuccessResult(this.name, from, this._inferChainId(rpcUrl));
    } catch (err) {
      return {
        status: "unavailable",
        balanceChanges: [],
  ...
  /**
   * Best-effort chain ID inference from RPC URL.
   */
  private _inferChainId(_rpcUrl?: string): number {
    return 0;
  }
}
